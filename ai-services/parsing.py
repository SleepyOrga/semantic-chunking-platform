...
class DOLPHIN:
    def __init__(self, model_id, max_tokens: int = 2048):
        ...
        # process prompt
        self.tokenizer = self.model.llm_engine.get_tokenizer_group().tokenizer
    ...
    def chat(self, prompt, image):
        ...
        for i, p in enumerate(prompts):
            ...
            decoder_prompt = f"<s>{p.strip()} <Answer/>"
            image = images[i]
            enc_dec_prompt = ExplicitEncoderDecoderPrompt(
                ...
                decoder_prompt=TokensPrompt(
                    prompt_token_ids=self.tokenizer(decoder_prompt, add_special_tokens=False)["input_ids"]
                ),
            )
            all_prompts.append(enc_dec_prompt)
        ...
        for output in outputs:
            decoder_prompt_tokens = self.tokenizer.batch_decode(output.prompt_token_ids, skip_special_tokens=True)
