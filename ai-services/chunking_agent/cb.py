...
from pdf_preprocessing_steps import PDFPreprocessor
from word_preprocessing_steps import WordPreprocessor
from ocr_preprocessing_steps import OCRPreprocessor
...
class DocumentPreprocessor:
    """Unified preprocessor that chooses the right preprocessing based on source type"""
    
    def __init__(self):
        self.excel_preprocessor = ExcelPreprocessor()
        self.pdf_preprocessor = PDFPreprocessor()
        self.word_preprocessor = WordPreprocessor()
        self.ocr_preprocessor = OCRPreprocessor()
    
    def preprocess_content(self, content: str, source_type: str = None, 
                          source_file_path: str = None) -> tuple[str, dict]:
        """
        Preprocess content based on source document type
        
        Args:
            content: Markdown content to preprocess
            source_type: Type of source document ('excel', 'pdf', 'word', 'ocr')
            source_file_path: Path to original source file
        
        Returns:
            (preprocessed_content, preprocessing_metadata)
        """
        
        # Auto-detect source type if not provided
        if not source_type and source_file_path:
            source_type = self._detect_source_type(source_file_path, content)
        
        print(f"[{time.time():.2f}] Preprocessing content as {source_type} type")
        
        # Apply appropriate preprocessing
        if source_type == 'excel':
            return self.excel_preprocessor.preprocess_excel_markdown(content)
        elif source_type == 'pdf':
            return self.pdf_preprocessor.preprocess_pdf_markdown(content)
        elif source_type == 'word':
            return self.word_preprocessor.preprocess_word_markdown(content)
        elif source_type == 'ocr':
            return self.ocr_preprocessor.preprocess_ocr_markdown(content)
        else:
            # No preprocessing needed
            return content, {'preprocessing_steps': ['none'], 'source_type': 'unknown'}
    
    def _detect_source_type(self, file_path: str, content: str) -> str:
        """Auto-detect source document type"""
        if file_path:
        ...
        excel_indicators = [
            ...
            len(re.findall(r'\$[\d,]+', content)) >= 3,
            len(re.findall(r'\d+%', content)) >= 2
        ]
        
        if sum(excel_indicators) >= 2:
            return 'excel'
        
        # Check for OCR indicators
        ocr_indicators = [
            len(re.findall(r'[^\w\s.,!?;:()-]{2,}', content)) > 5,
            len(re.findall(r'\w{15,}', content)) > 3
        ]
        
        if sum(ocr_indicators) >= 1:
            return 'ocr'
        
        return 'unknown'

# Enhanced version of your chunker with preprocessing
class EnhancedMarkdownSemanticChunkerWithPreprocessing(EnhancedMarkdownSemanticChunker):
    """
    Your existing chunker enhanced with preprocessing capabilities
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Add file-type specific patterns to your existing pattern_to_label
        self.pattern_to_label.update({
            # Excel-specific patterns
            r'(?i)revenue|profit|margin|kpi|metrics': 'Business Metrics',
            r'(?i)quarter|q[1-4]|monthly|annual|ytd': 'Time Period',
            r'\$[\d,]+|\€[\d,]+|£[\d,]+': 'Financial Data',
            r'\d+%': 'Percentage Data',
            r'(?i)table|chart|graph|figure': 'Data Visualization',
            
            # PDF-specific patterns
            r'(?i)page \d+|chapter \d+': 'Document Structure',
            r'(?i)confidential|proprietary': 'Document Metadata',
            
            # OCR-specific patterns
            r'<!-- LOW_CONFIDENCE -->': 'OCR Uncertain',
            r'<!-- POTENTIAL_HEADER -->': 'Possible Header'
        })
    
    def extract_chunks_with_preprocessing(self, file_path: str, 
                                        source_type: str = None,
                                        source_file_path: str = None) -> List[Dict[str, Any]]:
        """
        Extract chunks with preprocessing
        ...
        """
        start_time = time.time()
        logger.info(f"Processing file with preprocessing: {file_path}")
        ...
        enhanced_content, preprocessing_metadata = self.preprocessor.preprocess_content(
            original_content, source_type, source_file_path
        )
        preprocessing_time = time.time() - preprocessing_start
        logger.info(f"Preprocessing completed in {preprocessing_time:.2f}s")
        logger.info(f"Applied steps: {', '.join(preprocessing_metadata['preprocessing_steps'])}")
        
        # Temporarily save enhanced content
        import tempfile
        ...
        try:
            # Use your existing chunking logic on enhanced content
            chunks = self.extract_chunks(temp_path)
            
            # Add preprocessing metadata to chunks
            for chunk in chunks:
                chunk['preprocessing_metadata'] = preprocessing_metadata
                chunk['source_type'] = source_type or preprocessing_metadata.get('source_type', 'unknown')
                chunk['processed_by'] = 'hanhiho'
                chunk['processed_at'] = '2025-07-27 16:07:29'
                
                # Apply source-specific enhancements
                if source_type == 'excel':
                    if any(pattern in chunk['content'] for pattern in ['$', '€', '£']):
                        chunk['tags'] = chunk.get('tags', []) + ['Financial Data']
                    if '|' in chunk['content'] and chunk['content'].count('|') > 4:
                        chunk['tags'] = chunk.get('tags', []) + ['Table Data']
            
            total_time = time.time() - start_time
            logger.info(f"Total processing with preprocessing completed in {total_time:.2f}s")
...
def process_document_with_preprocessing(
    ...
    **chunker_kwargs
    ...
    """
    Process document with intelligent preprocessing
    
    Args:
        markdown_file_path: Path to converted markdown file
        source_file_path: Path to original source file
        source_type: Type of source document ('excel', 'pdf', 'word', 'ocr')
        output_file: Path to save output JSON
        **chunker_kwargs: Additional arguments for chunker
    ...
    use_llm = chunker_kwargs.pop('use_llm', True)
    max_processing_time = chunker_kwargs.pop('max_processing_time', 30)
    bedrock_model_id = chunker_kwargs.pop('bedrock_model_id', "anthropic.claude-3-5-sonnet-20241022-v2:0")
    
    # Initialize enhanced chunker
    chunker = EnhancedMarkdownSemanticChunkerWithPreprocessing(
        max_chunk_size=1500,
        min_chunk_size=200,
        use_llm=use_llm,
        bedrock_model_id=bedrock_model_id,
    ...
    chunks = chunker.extract_chunks_with_preprocessing(
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Extract semantic chunks from a Markdown file with preprocessing.")
    parser.add_argument("file_path", type=str, help="Path to the Markdown file")
    parser.add_argument("--source_file_path", type=str, help="Path to the original source file for preprocessing")
    parser.add_argument("--source_type", type=str, choices=['excel', 'pdf', 'word', 'ocr'], help="Type of source document")
    parser.add_argument("--output_file", type=str, default="markdown_chunks.json", help="Output JSON file path")
    parser.add_argument("--use_llm", action='store_true', help="Use LLM for chunk enhancement")
    parser.add_argument("--max_processing_time", type=int, default=30, help="Maximum processing time in seconds")
    ...
    chunks = process_document_with_preprocessing(
        markdown_file_path=args.file_path,
